...
int Floyd_Algo(int adg[length][length], int v1, int v2) {
    ...
    // Initializes the temporary distance matrix
    for(int i = 0; i < length; i++) {
        for(int j = 0; j < length; j++) {
            temp[i][j] = adg[i][j]; // Copies the adjacency matrix to the temporary distance matrix
        }
    }
    // Iterates over all vertices as intermediate points
    for(int k = 0; k < length; k++) {
        for(int i = 0; i < length; i++) {
            for(int j = 0; j < length; j++) {
                // Checks if a shorter path exists by using vertex k
                if(temp[i][k] + temp[k][j] < temp[i][j] || (temp[i][j] == 0 && i != j && temp[i][k] != 0 && temp[k][j] != 0)) {
                    temp[i][j] = temp[i][k] + temp[k][j]; // Update the shortest path
                }
            }
        }
    }
